function editData(id) {
    $("#submitbtn").html('Save જામીન લાયક વોરંટ'); //Change button value to add
    $.ajax({
        url: "/api/ApiSamans_jamin_details/GetJaminById",
        type: "Get",
        dataType: "json",
        data: { "id": id },
        headers: {
        ...
        success: function (response) {
            ...
            $('#form_Jamin_Details').trigger("reset");
            $('#basicModal').modal('show');
            Populate('#form_Jamin_Details', response.content);
        },
    });
}
...
$(document).ready(function () {
    ...
    $("#addData").click(() => {
        $('#form_Jamin_Details').trigger('reset');
        $("#submitbtn").html('+ Add જામીન લાયક વોરંટ'); //Change button value to add
    });

    $("#searchButton").click(function () {
        $('#data_Jamin_Details').DataTable().ajax.reload();
    });

    $('#data_Jamin_Details').DataTable({
        "processing": true,
        "language": {
        ...
        bAutoWidth: false,
        "ajax": {
            url: "/api/ApiSamans_jamin_details/GetJamin",
            type: "Get",
            dataSrc: "content",
            dataType: "json",
            data: function (d) {
    ...
    $("#form_Jamin_Details").validate({
        ...
        submitHandler: () => {
            var formData = new FormData(document.getElementById("form_Jamin_Details"));
            var payload = JSON.stringify(Object.fromEntries(formData));
            console.log(payload);
            $.ajax({
                url: '/api/ApiSamans_jamin_details/SaveJamin',
                type: 'post',
                dataType: 'json',
                headers: {
                    'Accept': 'application/json',
