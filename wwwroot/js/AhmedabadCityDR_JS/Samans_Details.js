function editData(id) {
    $("#submitbtn").html('Save સમન્‍સ'); //Change button value to add
    $.ajax({
        url: "/api/ApiSamans_jamin_details/GetSamansById",
        type: "Get",
        dataType: "json",
        data: { "id": id },
        headers: {
        ...
        success: function (response) {
            ...
            $('#form_Samans_Details').trigger("reset");
            $('#basicModal').modal('show');
            Populate('#form_Samans_Details', response.content);
        },
    });
}
...
$(document).ready(function () {
    ...
    $("#addData").click(() => {
        $('#form_Samans_Details').trigger('reset');
        $("#submitbtn").html('+ Add સમન્‍સ'); //Change button value to add
    });

    $("#searchButton").click(function () {
        $('#data_Samans_Details').DataTable().ajax.reload();
    });

    $('#data_Samans_Details').DataTable({
        "processing": true,
        "language": {
        ...
        bAutoWidth: false,
        "ajax": {
            url: "/api/ApiSamans_jamin_details/GetSamans",
            type: "Get",
            dataSrc: "content",
            dataType: "json",
            data: function (d) {
        ...
        "columns": [
            ...
            { title: 'તારીખ', data: 'createdDate', "width": "50px" },
    ...
    $("#form_Samans_Details").validate({
        ...
        submitHandler: () => {
            var formData = new FormData(document.getElementById("form_Samans_Details"));
            var payload = JSON.stringify(Object.fromEntries(formData));
            console.log(payload);
            $.ajax({
                url: '/api/ApiSamans_jamin_details/SaveSamans',
                type: 'post',
                dataType: 'json',
                headers: {
                    'Accept': 'application/json',
