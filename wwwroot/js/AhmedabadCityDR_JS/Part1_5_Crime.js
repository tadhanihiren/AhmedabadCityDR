function EditData(id) {
    $("#prohibitionCat").show();
    $("#submitbtn").html('Save પાર્ટ-એ'); //Change button value to Save
    //console.log("Edit popup show.");
    $.ajax({
        url: "/api/ApiPart1_5_Crimes/GetById",
        type: "Get",
        dataType: "json",
        data: { "id": id },
        headers: {
        ...
        success: function (response) {
            ...
            $('#form_Part1_5Crime').trigger("reset");
            $('#basicModal').modal('show');
            Populate('#form_Part1_5Crime', response.content);
        },
    });
}
...
function DeleteData(id) {
    swal({
        ...
        if (isConfirm) {
            $.ajax({
                url: "/api/ApiPart1_5_Crimes/Delete",
                type: "get",
                dataType: "json",
                data: { "id": id },
                headers: {
                ...
                success: function (response) {
                    ...
                    $('#data_Part1_5_Crime').DataTable().ajax.reload();
...
$(document).ready(() => {
    ...
    $("#addData").click(() => {
        $("#prohibitionCat").hide();
        $('#form_Part1_5Crime').trigger('reset');
        $("#submitbtn").html('+ ADD પાર્ટ-એ'); //Change button value to add
    });

    $("#subCategoryId").change(function () {
        var selectedValue = $('option:selected', this).val();
        if (selectedValue == 122 || selectedValue == 123 || selectedValue == 124 || selectedValue == 126) {
            $("#prohibitionCat").show();
        }
        else {
            $("#prohibitionCat").hide();
    ...
    $("#form_Part1_5Crime").validate({
        ...
        submitHandler: () => {
            var formData = new FormData(document.getElementById("form_Part1_5Crime"));
            var payload = JSON.stringify(Object.fromEntries(formData));
            //console.log(payload);
            $.ajax({
                url: "/api/ApiPart1_5_Crimes/Save",
                type: "post",
                dataType: "json",
                headers: {
                    Accept: "application/json",
