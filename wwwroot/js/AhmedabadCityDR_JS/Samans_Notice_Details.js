function editData(id) {
    $("#submitbtn").html('Save અન્ય નોટીસ'); //Change button value to add
    $.ajax({
        url: "/api/ApiSamans_jamin_details/GetNoticeById",
        type: "Get",
        dataType: "json",
        data: { "id": id },
        headers: {
        ...
        success: function (response) {
            ...
            $('#form_Notice').trigger("reset");
            $('#basicModal').modal('show');
            Populate('#form_Notice', response.content);
        },
    });
}
...
$(document).ready(function () {
    ...
    $("#addData").click(() => {
        $('#form_Notice').trigger('reset');
        $("#submitbtn").html('+ Add અન્ય નોટીસ'); //Change button value to add
    });

    $("#searchButton").click(function () {
        $('#data_Notice').DataTable().ajax.reload();
    });

    $('#data_Notice').DataTable({
        "processing": true,
        "language": {
        ...
        bAutoWidth: false,
        "ajax": {
            url: "/api/ApiSamans_jamin_details/GetNotice",
            type: "Get",
            dataSrc: "content",
            dataType: "json",
            data: function (d) {
    ...
    $("#form_Notice").validate({
        ...
        submitHandler: () => {
            var formData = new FormData(document.getElementById("form_Notice"));
            var payload = JSON.stringify(Object.fromEntries(formData));
            console.log(payload);
            $.ajax({
                url: '/api/ApiSamans_jamin_details/SaveNotice',
                type: 'post',
                dataType: 'json',
                headers: {
                    'Accept': 'application/json',
